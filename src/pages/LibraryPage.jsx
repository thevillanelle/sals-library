...
import { Search, Grid, List, Download, Star, ChevronLeft, ChevronRight, X, Layers, Plus } from 'lucide-react'
import { statusColors, statusLabels } from '../lib/bookUtils'
...
export default function LibraryPage() {
  ...
  useEffect(() => {
    supabase.from('books').select('author_last, user_books!inner(user_id)').eq('user_books.user_id', uid).order('author_last').then(({ data }) => {
      setAuthors([...new Set((data || []).map(b => b.author_last).filter(Boolean))])
    })
    supabase.from('books').select('genre, user_books!inner(user_id)').eq('user_books.user_id', uid).not('genre', 'is', null).then(({ data }) => {
      setGenres([...new Set((data || []).map(b => b.genre).filter(Boolean))].sort())
    })
  }, [uid])

  useEffect(() => { setCurrentPage(1) }, [debouncedSearch, filterStatus, filterRating, filterAuthor, filterGenre, filterFiction, sort, groupSeries])
      ...
      <div style={{ maxWidth: 1040, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <button onClick={() => setGroupSeries(g => !g)} title="Group by series" aria-label="Group by series" style={iconBtnStyle(groupSeries)}>
              <Layers size={14} />
            </button>
            <button onClick={() => setView(v => v === 'grid' ? 'list' : 'grid')} title={view === 'grid' ? 'List view' : 'Grid view'} aria-label={view === 'grid' ? 'Switch to list view' : 'Switch to grid view'} style={iconBtnStyle(view === 'list')}>
              {view === 'grid' ? <List size={14} /> : <Grid size={14} />}
            </button>
            <button onClick={exportExcel} title="Export to Excel" aria-label="Export to Excel" style={iconBtnStyle(false)}>
              <Download size={14} />
            </button>
        ...
        <div style={{ position: 'relative', marginBottom: 10 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search title, author, or series…" aria-label="Search title, author, or series"
